...
//절대 경로 예시 : 경로 조각 중 하나라도 절대 경로가 있으면 결과는 절대경로가됨(안쓰면 현재경로)
const pathJoin2 = path.join('/public', 'txt', 'file.txt');
console.log(pathJoin2);

//혼합경로 예시: ..을 사용하면 상위폴더로 이동(잘 사용안함)
const pathJoin3 = path.join("/users", 'martin', '../files', 'file.txt');
console.log(pathJoin3);

//resolve(경로들): 여러개의 경로조각을 결합하여 절대경로로 생성(경로가 없으면 현재경로) <- 사용 빈도 낮음
...
const filename = path.basename("/users/seyong/files/file.txt");
console.log(filename);
//.txt 파일 이름만 추출
const originalName = path.basename('/users/seyong/files/file.txt', '.txt');
console.log(originalName);

//extname(경로) : 주어진 경로의 확장자를 반환
const ext = path.extname("/users/seyong/files/file.txt");
console.log(ext);

//dirname(경로) : 주어진 경로의 디렉토리 부분을 반환
const dir = path.dirname("/users/seyong/files/file.txt");
console.log(dir);